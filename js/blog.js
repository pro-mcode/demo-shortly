...
function createBlogCard(blog) {
  ...
  card.innerHTML = `
    <div class="p-5">
      ...
      <p class="text-sm text-gray-600 mb-8 md:text-base">
        ${blog.description}
      </p>
      <div class="flex items-center gap-x-2 group cursor-pointer max-w-fit">
        <a href="#" class="text-sm text-black font-semibold md:text-base">Read More</a>
...
// ✅ Keep mobile detection, no resize re-check
function isMobileScreen() {
  return window.innerWidth < 800;
}

// ✅ Update visible cards only once on load (no dynamic resize)
function updateVisibleCards() {
  if (!isMobileScreen()) {
    // Desktop: show all
    blogs.forEach((blog) => (blog.style.display = "block"));
